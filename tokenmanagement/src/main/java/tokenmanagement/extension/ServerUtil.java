...
public class ServerUtil {
    ...
    private static final ModuleIdentifier TOKENMANAGEMENT_SUBSYSTEM = ModuleIdentifier.create("com.commerzunternahmen.tokenmanagement-subsystem");
    ...
    void addStepToUploadServerWar(OperationContext context) throws OperationFailedException {
        ...
        op.get("owner").set(new ModelNode().add("subsystem", SubsystemExtension.SUBSYSTEM_NAME));

        if (serverWar == null) {
            throw new OperationFailedException("TokenManagement Server WAR not found in keycloak-server-subsystem module");
        }
